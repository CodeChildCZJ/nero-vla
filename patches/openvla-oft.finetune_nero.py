...
def save_dataset_statistics(dataset_statistics, run_dir):
    """Local copy of `prismatic.vla.datasets.rlds.utils.data_utils.save_dataset_statistics`.

    Reimplemented here only because the original module imports the TensorFlow RLDS pipeline,
    which this NERO variant deliberately does not depend on. Output file is byte-identical in
    schema (`dataset_statistics.json`), so inference-time unnormalization is unchanged.
    """
    import json

    out_path = Path(run_dir) / "dataset_statistics.json"
...
class FinetuneConfig:
    ...
    # Dataset
    # Extracted NERO b2 frames. NERO_ROOT is the nero-vla repo root (see its env.example.sh).
    data_root_dir: Path = Path(os.environ.get("NERO_ROOT", ".")) / "bench/data/b2_oft"
    dataset_name: str = "nero_b2"                    # Name of fine-tuning dataset
    run_root_dir: Path = Path("runs")                # Path to directory to store logs & checkpoints
    shuffle_buffer_size: int = 100_000               # (unused by the NERO map-style loader; kept for run-id parity)
    # 111 train / 20 val episode split
    split_json: str = str(Path(os.environ.get("NERO_ROOT", ".")) / "bench/data/split.json")
    num_workers: int = 8                             # Dataloader workers (map-style loader, unlike RLDS)
    ...
    seed: int = 42                                   # Dataloader shuffling seed
...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    # Load Fine-tuning Dataset =>> NERO b2 is a LeRobot v2.1 real-robot dataset, not RLDS, so we take the
    # "bring your own torch Dataset" path the upstream script documents. `NeroChunkDataset` reproduces the
    # RLDS chunking / normalization / augmentation contract exactly (see prismatic/vla/datasets/nero_dataset.py).
    # Being map-style, it is finite, so the training loop below loops over epochs explicitly.
    ...
    assert not (set(train_episodes) & set(split["val_episodes"])), "train/val episode leak!"
    train_dataset = NeroChunkDataset(
        data_root=cfg.data_root_dir,
        episodes=train_episodes,
        batch_transform=batch_transform,
        image_size=tuple(vla.module.config.image_sizes)[0],
        image_aug=cfg.image_aug,
    )
    print(
        f"NERO train set: {len(train_episodes)} episodes / {len(train_dataset)} chunk anchors "
        f"(val episodes {split['val_episodes']} are held out entirely)"
    )
    assert not cfg.use_val_set, "Offline held-out scoring is done by scripts/oft_predict.py, not in-train val."
    ...
    sampler = (
        DistributedSampler(train_dataset, shuffle=True, seed=cfg.seed, drop_last=True)
    ...
    dataloader = DataLoader(
        ...
        collate_fn=collator,
        num_workers=cfg.num_workers,  # map-style loader (not RLDS), so torch parallelism is used
        drop_last=True,
    ...
    # The RLDS loader is an infinite stream; our map-style loader is finite, so cycle epochs here.
    def epoch_cycle():
        epoch = 0
        while True:
            ...
            for b in dataloader:
                yield b
            epoch += 1
            print(f"--- finished epoch {epoch} over {len(train_dataset)} anchors ---", flush=True)

    # Start training
    with tqdm.tqdm(total=cfg.max_steps, leave=False) as progress:
        vla.train()
        optimizer.zero_grad()
        for batch_idx, batch in enumerate(epoch_cycle()):
            ...
            # NOTE (deviation from upstream): upstream runs this block on EVERY step once warmup is
            # enabled, so after warmup it re-pins lr to original_lr each step -- which overwrites the
            # MultiStepLR 10x decay one step after the milestone fires (verified empirically).
            # Upstream never sees it because the official recipe leaves lr_warmup_steps=0. We use
            # both knobs, so warmup must hand the LR back to the scheduler once it is done.
            ...
            #
            # `log_step` is derived from `batch_idx // grad_accumulation_steps`, so it holds the
            # SAME value for every micro-batch of an accumulation group. Without the third clause
            # this block therefore fires `grad_accumulation_steps` times per save point -- with
            # accum=4 that is 4 full 7B LoRA merges (upstream itself warns the merge "can be very
            # slow") writing to the identical path, so 3 of the 4 are pure waste. Upstream never
            # hits it because the official ALOHA recipe is 8 GPUs x accum=1; it only bites at
            # accum > 1, i.e. exactly the single-GPU configuration.
            #
            # Guard on the FIRST micro-batch of the group (`% == 0`), not the last (`+1 % == 0`):
            # the `log_step == max_steps` break below fires at batch_idx == accum*max_steps, which
            # IS the first micro-batch, so a last-micro-batch guard would break out before the
            # final checkpoint was ever written. Saving on the first micro-batch also makes the
            # content match the label exactly -- weights after precisely `log_step` optimizer
            # updates, rather than log_step+1 as the unguarded last-of-group call produced.
            if (
                gradient_step_idx > 0
                and log_step % cfg.save_freq == 0
                and batch_idx % cfg.grad_accumulation_steps == 0
            ):
